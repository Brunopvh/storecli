...
if ! is_executable curl && ! is_executable wget && ! is_excutable aria2c; then
	_red "Instale uma das ferramentas a seguir ... curl, wget ou aria2c"
	exit 1
fi
...
_download_repo()
{
	...
	if command -v aria2c 1> /dev/null 2>&1; then
